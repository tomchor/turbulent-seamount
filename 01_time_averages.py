...
for j, config in enumerate(runs):
    ...
    aaai = adjust_times(aaai, round_times=True)
    #---
    ...
    if np.all(xyii.time.diff("time") > Δt_tol):
        ...
        aaai = aaai.reindex(dict(time=np.arange(0, xyzi.time[-1]+1e-5, Δt)), method="nearest", tolerance=Δt/Δt_tol)
    #---
