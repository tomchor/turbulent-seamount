...
if save_results:
    ...
    # Create dataset with fit results
    fit_ds = xr.Dataset({
        'amplitude': (['L'], amps),
        'power_law_exponent': (['L'], alphas),
        'transition_wavenumber': (['L'], k_transitions),
        'fit_success': (['L'], fit_successes),
    }, coords={
        'L': L_values
    })
...
if save_results:
    print(f"  - Output file: data_post/bathymetry_powerlaw_fits_{simname_base}.nc")
#---