...
title = @lift "α = $(@sprintf "%.2g" params.α), Frₕ = $(@sprintf "%.2g" params.Fr_h), Roₕ = $(@sprintf "%.2g" params.Ro_h), Sᴮᵘ = $(@sprintf "%.2g" params.Slope_Bu), Δz = $(@sprintf "%.2g" params.Δz_min) m;   Time = $(@sprintf "%s" prettytime(times[$n])) = $(@sprintf "%.3g" times[$n]/params.T_adv) adv periods = $(@sprintf "%.3g" times[$n]/params.T_inertial) Inertial periods"
